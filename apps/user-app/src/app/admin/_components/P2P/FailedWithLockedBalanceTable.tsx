      ...
      <div className="rounded-xl border-2 border-green-500/50 bg-green-950/30 p-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-green-200">No failed transfers with locked balance</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="overflow-hidden rounded-xl border border-slate-700 bg-slate-800 shadow-sm">
      <div className="border-b border-slate-700 bg-slate-800/50 px-5 py-3.5">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-slate-100">Failed Transfers with Locked Balance</h3>
            <p className="text-xs text-slate-400">Balance remains locked after failure</p>
          </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-700 bg-slate-800">
              ...
              <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Transfer Amount</th>
              <th className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Locked Balance</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-700 bg-slate-800">
            {rows.map((r: any, idx: number) => (
              <tr key={r.id} className={`transition-colors hover:bg-slate-700/50 ${idx % 2 === 0 ? 'bg-slate-800' : 'bg-slate-800/50'}`}>
                ...
                <td className="px-5 py-3.5">
                  <span className="inline-flex items-center gap-1.5 rounded-full bg-red-900/50 px-2.5 py-1 text-xs font-semibold text-red-300">
